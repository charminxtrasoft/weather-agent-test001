...
def get_best_model(client: Groq) -> str:
    """Finds the first supported active model on your Groq key."""
    try:
...
if st.button("Check Running Conditions", type="primary"):
    if not groq_key:
        st.error("Missing Groq API Key! Add GROQ_API_KEY in Streamlit Secrets or sidebar.")
    else:
        ...
        if not coords:
            ...
            st.caption(f"📍 Location: **{city_name}, {country}** ({lat:.2f}, {lon:.2f})")
            ...
            with st.spinner("Selecting model & evaluating weather safety..."):
                try:
